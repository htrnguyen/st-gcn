...
def read_data_and_pre_norm(file, max_body=4, num_joint=25):
    fp = np.zeros((1, 3, max_frame, num_joint, max_body_true), dtype=np.float32)
    data = read_xyz(file, max_body=max_body_kinect, num_joint=num_joint)
    fp[0, :, 0:data.shape[1], :, :] = data
    print("Chuan hoa khung xuong dau vao")
